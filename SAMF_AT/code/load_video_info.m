function [img_files, pos, target_sz, ground_truth, video_path] = load_video_info(base_path, dataset, video)
%LOAD_VIDEO_INFO
...
%   http://www.isr.uc.pt/~henriques/
	...
	if numel(video) >= 2 && video(end-1) == '-' && ~isnan(str2double(video(end))),
		suffixdash = video(end-1:end);  %remember the suffix
		video = video(1:end-2);  %remove it from the video name
	else
		suffixdash = '';
	end
	    if numel(video) >= 2 && video(end-1) == '.' && ~isnan(str2double(video(end))),
		suffixdot = video(end-1:end);  %remember the suffix
		video = video(1:end-2);  %remove it from the video name
	else
		suffixdot = '';
	end
